/*
 * ESP32-4848S040 - V8: Full Train Departure Board
 *
 ...
 * Columns: TIME, PLT, STATUS, LATE (minutes delay)
 * Color coded: green=on time, yellow=delayed, red=cancelled
 * Auto-refreshes every 60 seconds.
 *
 * Two API calls per direction to cover 3-hour window:
 *   Call 1: timeOffset=-60, timeWindow=60  (trailing hour)
 *   Call 2: timeOffset=0,   timeWindow=120 (next 2 hours)
 *   Results merged and deduplicated by serviceID.
 *
 * Required Libraries:
...
#define COL_LATE_HIGH   0xFBE0   // Orange
...
#define COL_TIME_X     3
#define COL_PLT_X      66
#define COL_STATUS_X   95
#define COL_LATE_X     197
...
struct TrainInfo {
    ...
    char etd[16];        // Estimated: "On time", "HH:MM", "Cancelled", "Delayed"
    char platform[4];
    bool isCancelled;
...
// Check if a serviceID already exists in the board
...
bool fetchAndAppend(const char *fromCrs, const char *toCrs, BoardData &board,
    ...
    // Parse JSON
    DynamicJsonDocument doc(49152);  // 48KB
    ...
    // Extract update time (only if not already set)
    ...
    if (services.isNull()) {
        ...
        return true;  // Valid response, no trains
    }
    ...
    for (JsonObject svc : services) {
        ...
        if (serviceExists(board, sid)) continue;  // Deduplicate
            ...
            for (JsonVariant cpListVar : cpLists) {
                ...
                for (JsonObject cp : cpList) {
                    ...
                    if (strcmp(crs, toCrs) == 0) {
                        ...
                        if (cpCancelled) {
                            t.delayMins = -1;
                        } else if (strcmp(et, "On time") == 0) {
                            t.delayMins = 0;
                        } else if (strlen(at) >= 5 && strlen(st) >= 5) {
                            // Actual arrival time available
                            t.delayMins = calcDelay(st, at);
                        } else if (strlen(et) >= 5 && strlen(st) >= 5) {
                            // Estimated arrival time
                            t.delayMins = calcDelay(st, et);
                        }
                        break;
                    }
                }
        ...
        Serial.printf("  %s Plt %s %s delay=%d\n", t.std, t.platform, t.etd, t.delayMins);
        board.count++;
    }
...
bool fetchDirection(const char *fromCrs, const char *toCrs, BoardData &board) {
    ...
    // Call 1: trailing hour (offset=-60, window=60)
    bool ok1 = fetchAndAppend(fromCrs, toCrs, board, -60, 60);

    delay(300);

    // Call 2: next 2 hours (offset=0, window=120)
...
void drawColumnHeaders() {
    for (int side = 0; side < 2; side++) {
        ...
        gfx->setCursor(xOff + COL_TIME_X, COLHDR_Y + 6);
        gfx->print("TIME");
        gfx->setCursor(xOff + COL_PLT_X, COLHDR_Y + 6);
        gfx->print("PLT");
        gfx->setCursor(xOff + COL_STATUS_X, COLHDR_Y + 6);
        gfx->print("STATUS");
        gfx->setCursor(xOff + COL_LATE_X, COLHDR_Y + 6);
        gfx->print("LATE");
    }
...
bool isTrainInPast(const TrainInfo &train) {
    ...
    if (nowMins < 0) return false;  // NTP not available
    ...
    if (diff > 720) diff -= 1440;
    if (diff < -720) diff += 1440;
    return (diff < -2);  // More than 2 mins ago
}
...
void drawTrainRow(int side, int row, const TrainInfo &train) {
    ...
    gfx->setCursor(xOff + COL_TIME_X, y + 3);
    gfx->print(train.std);
    ...
    if (train.isCancelled || strcmp(train.etd, "Cancelled") == 0) {
        ...
        statusColor = COL_DELAYED;
    }
    if (past) statusColor = COL_PAST_FG;

    gfx->setTextColor(statusColor);
    gfx->setCursor(xOff + COL_STATUS_X, y + 3);
    ...
    if (train.isCancelled || strcmp(train.etd, "Cancelled") == 0) {
        ...
        // Show estimated time (already "HH:MM" format)
        char buf[6];
        strlcpy(buf, train.etd, sizeof(buf));
        gfx->print(buf);
    }

    // LATE column
    gfx->setCursor(xOff + COL_LATE_X, y + 3);
    if (train.delayMins == -1) {
        // Cancelled
        ...
        gfx->setTextColor(past ? COL_PAST_FG : COL_ON_TIME);
        gfx->print("0");
    } else {
        // Late by N minutes
        uint16_t lateColor;
        if (train.delayMins >= 10) {
            lateColor = COL_CANCELLED;    // Red for 10+ min
        } else if (train.delayMins >= 5) {
            lateColor = COL_LATE_HIGH;    // Orange for 5-9 min
        } else {
            lateColor = COL_DELAYED;      // Yellow for 1-4 min
        }
        if (past) lateColor = COL_PAST_FG;
        gfx->setTextColor(lateColor);
        char buf[6];
        snprintf(buf, sizeof(buf), "+%d", train.delayMins);
        gfx->print(buf);
    }
}
...
void drawLoadingOverlay() {
    // Semi-transparent loading indicator in center
...
void setup() {
    Serial.begin(115200);
    Serial.println("V8: Full Train Board - Starting...");
